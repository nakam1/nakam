...
CAmount CMasternode::GetMasternodeNodeCollateral(int nHeight) 
{
    ...
    if(nHeight >    501) return     1000 * COIN;
...
CAmount CMasternode::GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 574010 || nHeight == 585330 || nHeight == 586594) return 1 * COIN;

    if(nHeight > 15000000) return    0 * COIN;
...
{
    if(nHeight > 1200000) return GetBlockValue(nHeight) * 60 / 100;

    return GetBlockValue(nHeight) * 60 / 100;
}
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when 1000 NAKAM tx got MASTERNODE_MIN_CONFIRMATIONS
    ...
    if (mi != mapBlockIndex.end() && (*mi).second) {
        ...
        int nConfHeight = pMNIndex->nHeight + MASTERNODE_MIN_CONFIRMATIONS - 1; // block for 1000 NAKAM tx -> 1 confirmation
