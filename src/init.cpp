...
void PrepareShutdown()
{
    ...
    util::ThreadRename("nakam-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all NAKAM specific functionality (Masternodes) (0-1, default: %u)"), 0));
...
{
    util::ThreadRename("nakam-loadblk");
...
/** Sanity checks
 *  Ensure that Nakam is running in a usable environment with all
...
bool AppInit2()
{
    ...
    // Sanity check
    if (!InitSanityCheck())
        return UIError(_("Initialization sanity check failed. Nakam is shutting down."));

    std::string strDataDir = GetDataDir().string();

    // Make sure only a single Nakam process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return UIError(strprintf(_("Cannot obtain a lock on data directory %s. Nakam is probably already running."), strDataDir));
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                // Nakam: load previous sessions sporks if we have them.
