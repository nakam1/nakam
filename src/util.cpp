...
const char * const PIVX_CONF_FILENAME = "nakam.conf";
const char * const PIVX_PID_FILENAME = "nakam.pid";
...
// Mac: ~/Library/Application Support/nakam
// Unix: ~/.nakam
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Nakam";
#else
    fs::path pathRet;
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty nakam.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override nakam.conf
