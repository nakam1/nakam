...
#if defined(NDEBUG)
#error "Nakam cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uNAKAM) are considered zero fee (for relaying, mining and transaction creation)
...
{
    util::ThreadRename("nakam-scriptch");
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived, CConnman& connman, std::atomic<bool>& interruptMsgProc)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // Nakam: We use certain sporks during IBD, so check to see if they are
