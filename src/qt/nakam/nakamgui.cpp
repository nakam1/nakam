...
#include "qt/nakam/defaultdialog.h"
#include "qt/nakam/settings/settingsfaqwidget.h"
...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("Nakam client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("Nakam"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("Nakam"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
