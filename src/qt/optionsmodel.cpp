...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    ...
    if (settings.contains("nAnonymizeNakamAmount") || reset)
        SoftSetArg("-anonymizenakamamount", settings.value("nAnonymizeNakamAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in nakam.cpp during shotdown to also remove the window positions
